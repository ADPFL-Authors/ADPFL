...
# Fani server
# sys.path.insert(0, os.path.abspath("/home/eric/ppfl/"))# Change all paths the same as this path
# sys.path.insert(0, os.path.abspath("/Users/ericlong/PycharmProjects/ppfl"))
...
from fedml_api.standalone.spdfl.spdfl_api import spdflAPI
from fedml_api.standalone.spdfl.spdfl_api_sch import spdflAPIsch
...
from fedml_api.standalone.spdfl.spdfl_model_trainer import spdflMT
...
def custom_model_trainer(args, model, logger):
    return spdflMT(model, args, logger)
...
if __name__ == "__main__":
    ...
    parser = add_args(argparse.ArgumentParser(description="spdfl-standalone"))
    ...
    # log_path = '/nfs/ppfl/fedml_spdfl/spdfl/LOG/' + args.dataset + '/' + args.identity + '.log'
    log_path = (
        "/nfs/ppfl/fedml_spdfl/spdfl/LOG/" + args.dataset + "/" + args.identity + ".log"
    )
    ...
    if args.new_schedule:
        print("new schedule")
        spdflAPIsch = spdflAPIsch(dataset, device, args, model_trainer, logger)
        spdflAPIsch.train()
    else:
        print("old schedule")
        spdflAPI = spdflAPI(dataset, device, args, model_trainer, logger)
        spdflAPI.train()
