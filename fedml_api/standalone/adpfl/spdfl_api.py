import torch
from fedml_api.standalone.adpfl.client import client
import copy, math
import numpy as np
from torch.nn.functional import cosine_similarity

# Import the functions from spdfl/sp_functions.py such as SparsityIndex
from fedml_api.standalone.adpfl.sp_functions import *


class spdflAPI(object):
    ...
    def train(self):
        self.logger.debug("DEBUG: Entered the train method.")
        ...
        for round_idx in comm_round_iterable:
            ...
            for clnt_idx in range(self.args.client_num_in_total):
                ...
                if active_ths_rnd[clnt_idx] == 1:
                    if self.args.aggr_method == "by_mask":
                        ...
                        print("No such aggregation method!")
                else:
                    w_local_mdl = copy.deepcopy(w_per_mdls[clnt_idx])

                # client local training
                tst_results = client.local_test(w_local=w_local_mdl, train_or_test=True)
                tst_results_ths_round.append(tst_results)
                # Inside the client's training function, before starting the local training
                # sparsity =  compute_sparsity(w_local_mdl)  # Replace with your actual function to compute sparsity.
                ...
                test_local_metrics = client.local_test(
                    w_local=w_local_mdl, train_or_test=True, tstb4=True
                )
            ...
            if len(early_prune_record) == 1:
                pruning_time = self.generate_pruning_freq(
                    a=early_prune_record[0], c=self.args.reconfig_reduce
                )
            else:
                pruning_time = []
            ...
            # if early_prune_flag:
            #    early_prune_record.append(round_idx)
            #    if (early_prune_record[-1]-actual_prune_epoch) >= self.args.reconfig_interval:
            #        early_prune_flag = True
            #        actual_prune_epoch = round_idx + 1
            #        self.logger.info("Actual Prune at round {}".format(round_idx + 1))
            #    else:
            #        early_prune_flag = False # This is to avoid the case that the early prune threshold is reached but the reconfig interval is not reached.

            self._local_test_on_all_clients_before(tst_results_ths_round, round_idx)
            self._local_test_on_all_clients_aft(final_tst_results_ths_round, round_idx)
    ...
    def _aggregate_func_bypqsim(self, c_index, nei_indexs, w_per_mdls, scope="layer"):
        # Just consider layer scope first since layer is better
        current_model = copy.deepcopy(w_per_mdls[c_index])
        ...
        mask_current = self.model_trainer.get_mask(model=current_model)
        sparsity_index = SparsityIndex(self.args.p, self.args.q)
        gamma_sim = self.args.gamma_sim

        # Calculate the sparsity index of current client
        c_client = copy.deepcopy(self.client_list[c_index])
        c_client.model_trainer.set_model_params(current_model)
        si_i_current = sparsity_index.make_si_(
            model=c_client.model_trainer.model, mask=mask_current, scope=scope
        )
        si_i_current_tensor = torch.cat([v.view(1) for v in si_i_current.values()])
        # print(si_i_current)
        # Calculate the similarity between current client and its neighbors
        si_i_nei_tensor = []
        for nei_idx in nei_index_except_current:
            # Initialize the sparsity index of neighbors every time
            sparsity_index_temp = SparsityIndex(self.args.p, self.args.q)
            nei_model_weight = copy.deepcopy(w_per_mdls[nei_idx])
            mask_nei = self.model_trainer.get_mask(model=nei_model_weight)
            nei_client = copy.deepcopy(self.client_list[nei_idx])
            nei_client.model_trainer.set_model_params(nei_model_weight)
            si_i_nei = sparsity_index_temp.make_si_(
                model=nei_client.model_trainer.model, mask=mask_nei, scope=scope
            )
            si_i_nei_tensor.append(
                torch.cat([v_n.view(1) for v_n in si_i_nei.values()])
            )
            del nei_model_weight, mask_nei, nei_client, si_i_nei

        # Calculate the cosine similarity between current client and its neighbors. The last 1 is itself.
        cosine_similarities = [
            cosine_similarity(si_i_current_tensor, tensor_nei, dim=0).item()
            for tensor_nei in si_i_nei_tensor
        ] + [1.0]

        all_models = [
            copy.deepcopy(w_per_mdls[nei_index])
            for nei_index in nei_index_except_current
        ] + [copy.deepcopy(current_model)]

        print("Cosine Similarities between PQ-index are:", cosine_similarities)
        # Use the computed cosine similarities to compute the weights
        scaled_similarities = [
            math.exp(gamma_sim * cos_sim) for cos_sim in cosine_similarities
        ]
        # Normalize to get the weights
        sum_exp = sum(scaled_similarities)
        weights = [scaled_sim / sum_exp for scaled_sim in scaled_similarities]
        print("Aggregate Weights are:", weights)

        # Aggregate models
        aggregated_model = {
            key: torch.zeros_like(tensor) for key, tensor in current_model.items()
        }
        for key in aggregated_model.keys():
            for i in range(len(weights)):
                aggregated_model[key] += weights[i] * all_models[i][key]
        del current_model
    ...
    def _aggregate_func_bymask(self, c_index, nei_indexs, w_per_mdls):
        """
