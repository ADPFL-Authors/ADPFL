...
from fedml_api.standalone.dfedsam.client import Client


class DFedSAMAPI(object):
    ...
    def train(self):
        ...
        for round_idx in comm_round_iterable:
            ...
            for clnt_idx in range(self.args.client_num_in_total):
                ...
                client_indexes = self._get_neighbors_of_client(expander_graph, clnt_idx)
                client_indexes = np.sort(client_indexes)

                self.logger.info("client_indexes = " + str(client_indexes))

                # aggregate the updates from the neighbors, which is float16 in this case.
                w_agg_temp = self._aggregate(
                    [
                        (self.client_list[i].get_sample_number(), w_per_mdls[i])
                        for i in client_indexes
                    ]
                )
                w_per_clnt_idx, training_flops, num_comm_params = client.train(
                    copy.deepcopy(w_agg_temp), round_idx
                )
                w_per_mdls_temp[clnt_idx] = copy.deepcopy(w_per_clnt_idx)
                self.stat_info["sum_training_flops"] += training_flops
                self.stat_info["sum_comm_params"] += num_comm_params

            w_per_mdls = copy.deepcopy(w_per_mdls_temp)
            w_per_mdls_with_num = [
                (self.client_list[i].get_sample_number(), w_per_mdls[i])
