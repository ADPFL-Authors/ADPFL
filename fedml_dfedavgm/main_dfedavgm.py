...
from fedml_api.standalone.dfedavgm.dfedavgm_api import DFedAvgMAPI
from fedml_api.standalone.dfedavgm.dfedavgm_model_trainer import DFedAvgMModelTrainer
...
def add_args(parser):
    ...
    parser.add_argument(
        ...
        default=0.9,
        metavar="N",
        help="momentum used for DFedAvgM",
    )
...
def custom_model_trainer(args, model, logger):
    return DFedAvgMModelTrainer(model, args, logger)
...
if __name__ == "__main__":
    ...
    args.identity = "dfedavgm" + "-" + data_partition
    ...
    DFedAvgMAPI = DFedAvgMAPI(dataset, device, args, model_trainer, logger)
    DFedAvgMAPI.train()
